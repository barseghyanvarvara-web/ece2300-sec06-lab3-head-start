...
MODULE_BLOCK_REGEX = re.compile(
    r"\bmodule\b\s+([a-zA-Z_]\w*)\s*.*?endmodule",
    re.DOTALL
)
XPROP_MACRO_REGEX = re.compile(
    r"`ECE2300(?:_SEQ)?(?:_XPROP)?\d*\s*\(\s*([^,)]+).*?\)"
)
...
class VerilogChecker:
    ...
    def _preprocess_code(self, verilog_code: str) -> str:
        # This well-tested function remains the same
        output = []
    ...
    def check_content(self, verilog_content: str, filename: str) -> List[Dict]:
        ...
        processed_content = self._preprocess_code(verilog_content)
...
def extract_module_xprop_signals_from_file(file_content: str) -> Dict[str, List[str]]:
    """
    ...
    content_no_block = re.sub(r'/\*.*?\*/', '', file_content, flags=re.DOTALL)
    content_no_comments = re.sub(r'//.*?$', '', content_no_block, flags=re.MULTILINE)
    ...
    for match in MODULE_BLOCK_REGEX.finditer(content_no_comments):
        ...
        signals = XPROP_MACRO_REGEX.findall(module_body)
        
        if signals:
            if module_name not in results:
                results[module_name] = []
            results[module_name].extend(signals)
...
def main(args_list=None) -> Tuple[List[str], Dict[str, List[str]]]:
    ...
    while files_to_process_q:
        ...
        signals_in_file = extract_module_xprop_signals_from_file(content)

        # Add to dictonary
        if signals_in_file:
            for module, signals in signals_in_file.items():
    ...
    return (final_build_paths, module_to_xprop_signals)
