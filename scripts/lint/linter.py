...
class VerilogLinter(NodeVisitor):
    ...
    def __init__(self, config):
        ...
        self._xprop_macro_out_signals_found_by_regex = set()

        self.current_module_xprop = [] 
    ...
    def visit_ModuleDef(self, node):
        ...
        if (node.name in self._xprop_macro_out_signals_found_by_regex):
            self.current_module_xprop = self._xprop_macro_out_signals_found_by_regex.get(node.name)
        else:
            self.current_module_xprop = []
        ...
        if node.items:
            for item in node.items:
                ...
                if isinstance(item, (Always, Initial, Function, Task, GenerateStatement, SystemCall)):
                    ...
                    if item.left and isinstance(item.left, Lvalue) and item.left.var:
                        lhs_node = item.left.var
                    else:
                        #Continue to check RHS if possible, but overall assign is invalid
                        self._add_violation(Rules.BADLHS, item.left)
                    
                    if item.right and isinstance(item.right, Rvalue) and item.right.var:
                        rhs_node = item.right.var
                    else:
                        self._add_violation(Rules.BADRHS, item.right)
                    ...
                    if lhs_node and rhs_node:
                        ...
                        if not is_lhs_simple_target:
                            self._add_violation(Rules.COMPLEXLHS, item.left, type=type(lhs_node).__name__)
                        ...
                        if is_lhs_simple_target:
                            if not (is_rhs_simple_target or is_rhs_simple_literal):
                                ...
                                self._add_violation(Rules.COMPLEXRHS, item, detail_msg=detail_msg)

                elif isinstance(item, InstanceList):
                    if item.module.lower() in DISALLOWED_GATES:
                        self._add_violation(Rules.PRIMONLY, item, list_of_gates={', '.join(ALLOWED_GATES)})
                    # If the module is not in allowed or disallowed, there is a high probability it is a user defined module    
                    if item.module.lower() not in ALLOWED_GATES and item.module.lower() not in DISALLOWED_GATES:
                        self._add_violation(Rules.NOMODULE, item, module_name=item.module)
    ...
    def visit_Always(self, node):
        ...
        if isinstance(node, AlwaysFF):
            ...
            for sig_name, (assign_lineno, cond_type_str) in self._conditionally_assigned_signals_info.items():
                if sig_name not in self.current_module_xprop:
...
def main(args_list: Optional[List[str]] = None):
    ...
    parser.add_argument("-x", "--xprop", dest="xprop_list", default=None, help="JSON string of a dictionary mapping modules to signals that use Xprop.\n Example: '{\"module_a\": [\"sig1\", \"sig2\"]}'")
    ...
    xprop_dict = {} # Default to an empty dictionary
    if args.xprop_list:
        try:
            xprop_dict = json.loads(args.xprop_list)
            if not isinstance(xprop_dict, dict):
    ...
    for f_path in filelist:
        ...
        try:
            ...
            linter._xprop_macro_out_signals_found_by_regex = xprop_dict
            linter.visit(ast)
